...
- **Pixi standalone renders into a RenderTexture:** blend modes on the ROOT of a `renderer.render({ container, target })` call are ignored (an `erase` quad paints opaque white) and scene-attached display objects carry transform state that lands pixels at wrong coordinates. Always render a DETACHED temp `Container`/`Sprite` with the blend/positioned children inside it — see `restoreLayerRegionFromBlob` / `SelectionTool.stampTexture` / `commitScratchpad`. This bit the brush commit (BUG-19): `commitScratchpad` used to render the scene-attached scratchpad sprite, so committed strokes inherited the viewport pan/zoom and landed at `canvasPoint*zoom+pan` — invisible at zoom 1/pan 0, badly offset otherwise. Regression-guarded at a non-identity zoom in `tests/e2e/21-canvas-selection.spec.ts` 5.9.1.
