...
| **Mindmap Editor** | Interactive node-based mindmaps (`.mind` files, pure JSON — `MindmapFile` v1). Powered by `@xyflow/react` v12. Custom `MindmapNodeComponent` with inline label editing (no modals). Auto-layout via `lib/mindmap/index.ts` (recursive tree positioning, ancestor-chain cycle guard so a user-created cycle can't cause infinite recursion); `wouldCreateCycle` also rejects a cycle-forming connection at the UI layer. Users override layout by dragging (`manualPosition`). 50-entry undo/redo history. Auto-save ~3s + `Ctrl+S`. Title bar uses `InlineFileTitle` (same pattern as canvas/docx/spreadsheet). Indexed in search (node labels). Graph: teal hexagons. |
| **Spreadsheet Editor** | `.xlsx`/`.xls`/`.csv` (reads `.xlsm`/`.xlsb`/`.ods`/`.tsv` too) via `jspreadsheet-ce` grid, lazy-loaded with `jsuites`. SheetJS (`xlsx`) bridges file bytes ↔ an in-memory `SpreadsheetWorkbook` model (`lib/spreadsheet/`); `writeSpreadsheetFile` is format-aware (`bookTypeFromPath`) so `.csv` round-trips as `.csv` rather than becoming `.xlsx`. Multi-sheet tabs; switching sheets syncs the mounted grid back into the model first so in-progress edits aren't lost. Auto-save ~750ms. Indexed in search (`extractXlsxText`, cell text flattened to CSV per sheet, capped 14k chars). Graph: sharp rects. |
| **DOCX Editor** | `.docx` via `@eigenpal/docx-js-editor` (ProseMirror-based), lazy-loaded. Auto-save ~750ms. Responsive zoom via `ResizeObserver` writing a `--docx-zoom` CSS custom property (CSS `zoom`, not `transform`, so scroll stays correct at ~850px fixed page width). Mobile double-tap is manually forwarded as a synthetic `dblclick` for ProseMirror's word-select. **Not** indexed in search. **Unmount-save ref gotcha:** the editor instance is captured via a callback ref that ignores `null` detach calls, because React nulls JSX-bound refs during the unmount mutation phase — which runs before this component's own passive-effect cleanup fires — so a plain `ref={editorRef}` would always read `null` in the unmount flush-save. |
| **Code File Editor** | Plain-text/source files via CodeMirror 6 (`components/notes/code-file-editor.tsx`). Per-extension language support lazy-loaded (`lib/code/language-support.ts`) so unrelated `@codemirror/lang-*` packages aren't bundled together; unmapped extensions still open, just without highlighting. Theme (`lib/code/codemirror-theme.ts`) reads app CSS custom properties so it follows Light/Dark/System automatically. Auto-save 3s + blur + `Ctrl+S`. **Not** indexed in search. Graph: circles (same as plain notes). |
...
│   ├── File I/O Adapter
│   └── Search Index (node labels)
├── Spreadsheet Editor
│   ├── jspreadsheet-ce + jsuites (grid)
│   ├── SheetJS / xlsx (file bytes ↔ workbook model)
│   ├── File I/O Adapter
│   └── Search Index (cell text)
├── DOCX Editor
│   ├── @eigenpal/docx-js-editor (ProseMirror-based)
│   └── File I/O Adapter
├── Code File Editor
│   ├── CodeMirror 6 (@codemirror/*, per-extension lazy language support)
│   └── File I/O Adapter
├── File Browser View
│   ├── File I/O Adapter
│   └── Search Index
