...
| Chat | AI1 | Embeddings RAG | Vault chat v1 uses MiniSearch (lexical) for retrieval. True embeddings (Transformers.js + local vector store) deferred to tier 2 â€” better recall on paraphrased questions at the cost of a ~100MB model download. |
...
| Chat | AI3 | Local model download UX | Local (`device`) provider is Gemma 4 E2B only (MediaPipe/OPFS). Status row: while downloading, subtitle must not claim â€œcachedâ€; Ready uses green tick styling. Requires WebGPU. |
| Chat | AI4 | Tool calling / writes | Both chat surfaces are read-only â€” no document edits, file creation, or multi-step agents. Deferred until after vault chat UX settles. |
| Chat | AI5 | PDF rename â†’ asset-id | Out-of-band OS renames while the app is closed break the PDFâ†’`chatAssetId` mapping in `_marrow/_chats/index.json`. In-app renames are handled. Consider a periodic reconciliation pass. |
...
- [x] **File browser ? image thumbnails (F5)** ? Grid 56ï¿½56 / list 20ï¿½20; cache; rename refreshes thumbnail.
