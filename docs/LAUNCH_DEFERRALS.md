...
| Testing    | E2E1 | Mobile Playwright project rot   | The `mobile` project (Pixel 5) was not repaired in the 2026-07-09 chromium suite fix. Mobile uses different affordances (hamburger masthead, no Ctrl shortcuts, sheet nav), so fixtures need a mobile-aware `navigateTo` and specs need mobile-specific expectations. Until then, run E2E with `--project=chromium`. |
