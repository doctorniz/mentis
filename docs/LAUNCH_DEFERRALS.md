...
- [ ] **Markdown editor — find/replace** — Open a long note; **Ctrl+F** opens the floating find bar (top-right) with the current selection prefilled when one exists; the global vault-search view must **not** open. Typing highlights all matches (accent) with the active one distinct (warning); count shows `n/m`; Enter / Shift+Enter cycle next/prev and scroll the match into view; chevron on the left reveals the replace row — Replace swaps the active match, All swaps every match; Esc closes and clears highlights; the Search icon in the mode bar opens the same bar. In **Source** mode, Ctrl+F opens CodeMirror's own search panel instead (and also must not trigger vault search). Matches spanning bold/italic boundaries are found; matches never span across paragraphs.
- [ ] **Markdown editor — outline panel** — Right column shows an **Outline** section above Backlinks (chevron header, heading count badge). Headings H1–H6 listed with indentation by level, live-updating as you type (no save needed); click scrolls the editor to that heading smoothly. Collapsed state persists across reloads (`localStorage['ink-marrow:outline-collapsed']`). With chat and backlinks both collapsed, an expanded outline fills the column.
- [ ] **Markdown editor — mode-switch scroll** — Scroll midway into a long note in Visual mode; toggle to Source: the raw editor lands at roughly the same proportional position (not the top). Toggle back to Visual: position is again roughly preserved.
- [ ] **Markdown editor — image resize/alt** — Click an embedded image to select it (accent ring); drag the bottom-right handle to resize (live px readout; min 80px, capped at editor width); double-click the handle resets natural size. The **Alt** chip opens an inline input; Enter/✓ saves, Esc cancels. Switch to Source mode: a resized image reads `![alt|400](path)`; an image with alt text containing a non-numeric pipe (`![cats|dogs](x)`) is left intact. Reopen the note: width persists. Print/export uses the width.
